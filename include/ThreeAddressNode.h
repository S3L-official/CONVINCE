//
// Created by Pengfei Gao on 2020/2/12.
//

#ifndef SAFELANG_THREEADDRESSNODE_H
#define SAFELANG_THREEADDRESSNODE_H
...
public:
    ThreeAddressNode(string nodeName, const ThreeAddressNodePtr &lhs, const ThreeAddressNodePtr &rhs,
                     ASTNode::Operator op, NodeType nodeType) : op(op), lhs(lhs), rhs(rhs), nodeName(nodeName),
                                                                nodeType(nodeType) {
        SSAIndex = 0;
        lineNumber = 0;
...
//            assert(false);
    ...
    ThreeAddressNode(string nodeName, ThreeAddressNodePtr node) : nodeName(nodeName), lhs(node->lhs), rhs(node->rhs), op(node->op), nodeType(node->nodeType), distribution(node->getDistribution())  {
        lineNumber = 0;
        SSAIndex = 0;
        numberOfChild = 0;
...
//        if(lhs->getNodeName() == "res0@199(t323)@223(t26)") {
//            cout << "h" << endl;
//        }
        string oper;
        if(rhs != nullptr) {
            ...
            oper = getOper();
            result = getNodeName() + " = " + oper + " " + lhs->getNodeName();
        }
...
//            return;
    ...
    string prettyPrint2() {
        if(this->getNodeType() == NodeType::RANDOM) {
            ...
            return  " '" + this->getID() + " ";
        else if(this->getOp() == ASTNode::Operator ::NOT)
...
#endif //SAFELANG_THREEADDRESSNODE_H
