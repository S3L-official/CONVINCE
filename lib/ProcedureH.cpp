...
vector<set<ThreeAddressNodePtr>> ProcedureH::getY() {
    vector<set<ThreeAddressNodePtr>> res;
    set<ThreeAddressNodePtr> tempSet;
    auto arguments = this->getParameters();

    map<ThreeAddressNodePtr, set<ThreeAddressNodePtr>> nodeToParameters;

    for(auto ele : arguments) {
        for(auto ele1 : ele) {
            if(ele1->getNodeType() != NodeType::PARAMETER)
                continue;
            set<ThreeAddressNodePtr> temp {ele1};
            res.push_back(temp);
            tempSet.insert(ele1);
            nodeToParameters[ele1] = temp;
        }
    }
    const vector<ThreeAddressNodePtr>& block = this->getBlock();
    for(auto ele : block) {
        bool isBinary = ele->getNodeType() == NodeType::INTERNAL && tempSet.count(ele->getLhs()) != 0 && tempSet.count(ele->getRhs()) != 0;
        if(isBinary) {
            assert(nodeToParameters.count(ele->getLhs()) != 0);
            assert(nodeToParameters.count(ele->getRhs()) != 0);
            set<ThreeAddressNodePtr> temp;
            temp.insert(nodeToParameters[ele->getLhs()].begin(), nodeToParameters[ele->getLhs()].end());
            temp.insert(nodeToParameters[ele->getRhs()].begin(), nodeToParameters[ele->getRhs()].end());
            res.push_back(temp);
            tempSet.insert(ele);
            nodeToParameters[ele] = temp;
        }

        bool isUnitary = ele->getNodeType() == NodeType::INTERNAL && tempSet.count(ele->getLhs()) != 0 && !ele->getRhs();
        if(isUnitary) {
            set<ThreeAddressNodePtr> temp;
            temp.insert(nodeToParameters[ele->getLhs()].begin(), nodeToParameters[ele->getLhs()].end());
            res.push_back(temp);
